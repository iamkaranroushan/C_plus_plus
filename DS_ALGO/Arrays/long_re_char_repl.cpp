...
int longest_repeat_char_replacement(std::string& s, int k){
    ...
    while(right < s.size()){
        ...
        while(right - left + 1 - most_frequent_character_count > k){//means not a valid window.
...
int main(){
    std::string s = "CBBBAC";
    int k = 1;
    std::cout<<"maximum window size: "<<longest_repeat_char_replacement(s, k);
}
...
//         most_frequent_character_count = std::max(charcount[current_character], most_frequent_character_count);
        ...
        //now calculate if the window formed is a valid window. nad calculate the max window size at this point
//       //window_size = right - left + 1;not needed.
//         //max_window_size = std::max(max_window_size, window_size);should not be calculated here .
//         //characters_to_be_replaced = window_size - most_frequent_character_count;not needed.
//         while(characters_to_be_replaced > no_of_replacement){//means not a valid window.
            //means we have to shrink the window till the window becomes valid.
//             int left_char = s[left];
//             charcount[left_char]--;
//            // if(charcount[left_char] == most_frequent_character_count){    //condition not needed.
//              //   most_frequent_character_count--; should not be decreased manually 
//             //}
//             left++;
//         }
//         right++;
